...
/* tievie-fixes.js — v3.36 */
(function(){
  const VERSION = "v3.36 — streaming multi-select + IMDb edit";
  function ready(fn){ if(document.readyState!=="loading") fn(); else document.addEventListener("DOMContentLoaded", fn); }
  const SERVICES = ["Streamz","Netflix","Prime Video","Apple TV+","VRT MAX","Go Play","NPO","Disney+","Onbekend"];
  const STORAGE_KEY = "tievie_stream_multi_v4";
  function norm(s){
  ...
  function getSel(){ try{const v=JSON.parse(localStorage.getItem(STORAGE_KEY)||"[]"); return Array.isArray(v)?v:[];}catch{return [];} }
  ...
  function ensureSingleBadge(){
    ...
    tag.id='verBadge';
    Object.assign(tag.style,{position:'fixed',right:'12px',bottom:'12px',padding:'6px 10px',borderRadius:'10px',
  ...
  function createMultiDropdown(){
    ...
    SERVICES.forEach(s=>{ const id='svc_'+s.replace(/\W+/g,'_'); const row=document.createElement('label');
      Object.assign(row.style,{display:'flex',alignItems:'center',gap:'8px',padding:'6px 2px'});
      const cb=document.createElement('input'); cb.type='checkbox'; cb.value=s; cb.id=id;
    ...
    apply.addEventListener('click', ()=>{
      const chosen=[...list.querySelectorAll('input[type=checkbox]:checked')].map(cb=>cb.value);
      setSel(chosen); panel.style.display='none'; applyStreamFilter();
    });
  }
  ...
  function enableImdbInlineEdit(){
    ...
    cards.forEach(card=>{
      ...
      const m=(row.textContent||"").match(/IMDB\s*:\s*([0-9.]+|N\/[AB])/i);
      const cur=m?m[1]:'N/B';
      row.innerHTML = '<label style="font-weight:600;margin-right:6px">IMDB:</label>'+
                      '<input class="imdb-inline" value="'+(cur||'')+'" style="width:68px;padding:4px 6px;border:1px solid rgba(255,255,255,.25);border-radius:8px;background:rgba(255,255,255,.06);color:inherit"/>';
  ...
  function hookRender(){
    if(window.app && typeof app.render==='function' && !app.__patched36){
      app.__patched36 = true;
      ...
      setInterval(()=>{ enableImdbInlineEdit(); }, 1200);
    }
  }

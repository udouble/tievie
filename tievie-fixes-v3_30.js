...
/* Tievie fixes (v3.30) — Streaming multi-select overlay (vinkjes), enkel UI-laag.
   - Eén "Streaming ▾" naast Sorteren
   - Overlay (z-index 999999) met vinkjes + Alles/Geen/Omkeren + "Alles behalve"
   - Keuze onthouden (localStorage: tievie_stream_multi_v2)
   - Filtering gebeurt bovenop app.filtered(), data & logica blijven onaangeroerd
*/
(function(){
  ...
  function ensureFilterPatch(){
    if(typeof window.app!=="object"||typeof app.filtered!=="function") return;
    ...
    app.filtered = function(){
      ...
      return arr.filter(it=>{
        const norm = LABEL(NORMALIZE(it.streamingOn));
        const hit = sel.includes(norm);
  ...
  function hideOldStreamingSelects(){
    document.querySelectorAll("select").forEach(s=>{
      ...
      const looksLike = txt.includes("alle diensten") && txt.includes("netflix") && txt.includes("apple tv");
      if(looksLike){ s.style.display="none"; s.classList.add("tv-old-streaming-hidden"); }
    });
  }
  ...
  function buildOverlayButton(){
    ...
    btn.textContent="Streaming ▾"; zone.appendChild(btn);
    const chip=document.createElement("span"); chip.id="tvStreamChip"; chip.className="ml-2 text-sky-300 text-sm italic"; zone.appendChild(chip);
    btn.addEventListener("click", openOverlay); renderChip();
  }

  function openOverlay(){
    if(document.getElementById("tvStreamModal")) return;
    const st=getState(); const selected = new Set(Array.isArray(st.selected)?st.selected:[]);
    const wrap=document.createElement("div"); wrap.id="tvStreamModal"; wrap.innerHTML=`
      <div class="tv-modal-backdrop"></div>
      <div class="tv-modal" role="dialog" aria-modal="true" aria-label="Filter op streaming">
        <div class="tv-modal-head"><h3>Filter op streaming</h3><button type="button" id="tvClose" aria-label="Sluiten">×</button></div>
        <div class="tv-modal-body">
          <div class="tv-actions">
            ...
            <label class="tv-ex"><input type="checkbox" id="tvModeExclude"> Alles behalve</label>
          </div>
          <div class="tv-grid" id="tvList"></div>
        </div>
        <div class="tv-modal-foot"><button type="button" id="tvCancel">Annuleer</button><button type="button" id="tvApply" class="tv-apply">Toepassen</button></div>
      </div>`;
    document.body.appendChild(wrap);
    const css=`#tvStreamModal{position:fixed;inset:0;z-index:999999;display:flex;align-items:center;justify-content:center}
      .tv-modal-backdrop{position:absolute;inset:0;background:rgba(0,0,0,.55);backdrop-filter:saturate(120%) blur(2px)}
      .tv-modal{position:relative;width:min(560px,92vw);max-height:82vh;background:#0b1020;color:#e5e7eb;border:1px solid rgba(148,163,184,.25);
      ...
      .tv-modal-head h3{margin:0;font-size:1rem}.tv-modal-head button{background:transparent;border:0;color:#e5e7eb;font-size:1.25rem;line-height:1}
      .tv-modal-body{padding:12px 16px;overflow:auto}.tv-actions{display:flex;gap:8px;align-items:center;margin-bottom:10px;flex-wrap:wrap}
      ...
      .tv-ex{margin-left:auto;display:flex;align-items:center;gap:8px;font-size:.9rem}.tv-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}
    ...
    ORDER.forEach(name=>{const row=document.createElement("label"); row.innerHTML=`<input type="checkbox" ${selected.has(name)?"checked":""}> <span>${name}</span>`; list.appendChild(row)});
    wrap.querySelector("#tvModeExclude").checked=(st.mode==="exclude");
    wrap.addEventListener("click",(e)=>{
      if(e.target.id==="tvClose"||e.target.id==="tvCancel"||e.target.classList.contains("tv-modal-backdrop")){wrap.remove()}
      const act=e.target?.getAttribute?.("data-act"); if(act){const checks=wrap.querySelectorAll('#tvList input[type="checkbox"]');
        if(act==="all"){checks.forEach(c=>c.checked=true)} if(act==="none"){checks.forEach(c=>c.checked=false)} if(act==="invert"){checks.forEach(c=>c.checked=!c.checked)}}
      if(e.target?.id==="tvApply"){const picked=[]; wrap.querySelectorAll('#tvList input[type="checkbox"]').forEach(c=>{if(c.checked)picked.push(c.nextElementSibling.textContent.trim())});
        const mode=wrap.querySelector("#tvModeExclude").checked?"exclude":"include"; saveState({selected:picked,mode}); wrap.remove();
        if(typeof app?.render==="function") app.render(); renderChip();}
    }, true);
  }

  function init(){ ensureFilterPatch(); hideOldStreamingSelects(); buildOverlayButton(); }

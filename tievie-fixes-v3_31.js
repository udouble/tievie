...
/* Tievie fixes (v3.31) — Streaming multi-select overlay
   - Enkel 1 "Streaming ▾" knop (dup-verwijdering/hide select)
   - Overlay (z-index 999999) met vinkjes + Alles/Geen/Omkeren + "Alles behalve"
   - Keuze onthouden (localStorage: tievie_stream_multi_v2)
   - Robust filter patch: retry tot app.filtered bestaat, ook bij 'Toepassen'
*/
(function(){
  ...
  function ensureFilterPatchedWithRetry(){
    if(patchFilterOnce()) return;
    let tries = 0;
    const t = setInterval(()=>{
      tries++;
      if(patchFilterOnce() || tries>50){ clearInterval(t); }
    }, 100);
  }

  function hideLegacyStreamingControls(){
    // Verberg oude <select> met streaming opties
    ...
    // Verwijder per ongeluk dubbel geïnjecteerde buttons
    document.querySelectorAll("#tvStreamBtn").forEach((btn, i)=>{ if(i>0) btn.remove(); });
  }

  function findToolbar(){
    // Heuristiek 1: rij met sorteer-select
    ...
    // fallback: naast interne zoekveld
  ...
  function openOverlay(){
    ...
    wrap.addEventListener("click",(e)=>{
      ...
      if(e.target?.id==="tvApply"){
        const picked=[];
        wrap.querySelectorAll('#tvList input[type="checkbox"]').forEach(c=>{ if(c.checked) picked.push(c.nextElementSibling.textContent.trim()); });
        const mode=wrap.querySelector("#tvModeExclude").checked?"exclude":"include";
        saveState({selected:picked,mode});
        ensureFilterPatchedWithRetry();
